...
MSS:
	...
	if (nwKeyLen < SIZE_64_BYTES && psk_format) {
		...
		if (passhash(p_nwKey, nwKeyLen, (uchar *)info->ssid,
			strlen(info->ssid), output)) {
			TUTRACE((TUTRACE_ERR, "wps_createM8Sta: passhash failed\n"));
		}
		else {
			/*  hex to string */
			if (wps_hex2str(info->nwKey, sizeof(info->nwKey), output,
				SIZE_32_BYTES)) {
...
wps_get_upnpDevSSR(WPSAPI_T *g_mc, void *p_cbData, uint32 length, CTlvSsrIE *ssrmsg)
{
	...
	if (info->sc_mode == SCMODE_AP_REGISTRAR) {
		...
		/* Version */
		tlv_err |= tlv_dserialize(&ssrmsg->version, WPS_ID_VERSION, bufObj, 0, 0);
		/* Selected Registrar */
		tlv_err |= tlv_dserialize(&ssrmsg->selReg, WPS_ID_SEL_REGISTRAR, bufObj, 0, 0);
		/* Device Password ID */
		tlv_err |= tlv_dserialize(&ssrmsg->devPwdId, WPS_ID_DEVICE_PWD_ID, bufObj, 0, 0);
		/* Selected Registrar Config Methods */
		tlv_err |= tlv_dserialize(&ssrmsg->selRegCfgMethods, WPS_ID_SEL_REG_CFG_METHODS,
			bufObj, 0, 0);
		if (tlv_err) {
			ret = WPS_ERR_SYSTEM;
			goto err;
...
uint32
wps_createM8StaPskKey(void *g_mc)
{
	return wpsap_createM8Sta(g_mc, TRUE);
}
