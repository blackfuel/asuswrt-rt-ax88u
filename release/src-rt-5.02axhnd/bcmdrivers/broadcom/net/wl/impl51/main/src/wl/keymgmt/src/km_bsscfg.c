...
#ifdef BCMULP
void
km_bsscfg_ulp_init(keymgmt_t *km, const wlc_bsscfg_t *bsscfg, km_bsscfg_flags_t flags)
{
	km_bsscfg_t *bss_km;
...
km_bsscfg_get_max_static_config_size(keymgmt_t *km)
{
	...
	/* wsec */
	sz += KM_SERIAL_TLV_SIZE(sizeof(uint32));
	...
	sz += KM_SERIAL_TLV_SIZE(sizeof(bool));
	return sz;
}
	...
	const uint8 *buf, int len)
{
	...
	err = km_serial_parse_hdr(km, KM_SERIAL_OBJ_TYPE_BSSCFG_STATIC,
		KM_SERIAL_BSSCFG_STATIC_VERSION, buf, (size_t)len, &num_tlvs);
