...
reg_msg_nonce_parse(TlvEsNonce *t, uint16 theType, BufferObj *theBuf, BufferObj *authKey)
{
	...
	if (tlv_dserialize(&t->nonce, theType, theBuf, SIZE_128_BITS, 0)) {
	...
	if (tlv_dserialize(&t->keyWrapAuth, WPS_ID_KEY_WRAP_AUTH, theBuf, SIZE_64_BITS, 0)) {
		TUTRACE((TUTRACE_ERR, "De-serialize error!!\n"));
		return -1;
	}

	/* calculate the hmac of the data (data only, not the last auth TLV) */
	(void)hmac_sha2(HASH_SHA256, authKey->pBase, SIZE_256_BITS, NULL, 0, theBuf->pBase,
		hmac_len, dataMac, SIZE_256_BITS);
...
reg_msg_nonce_write(TlvEsNonce *t, BufferObj *theBuf, BufferObj *authKey)
{
	...
	(void)hmac_sha2(HASH_SHA256, authKey->pBase, SIZE_256_BITS, NULL, 0, theBuf->pBase,
		theBuf->m_dataLength, hmac, SIZE_256_BITS);
...
reg_msg_m7ap_parse(EsM7Ap *tlv, BufferObj *theBuf, BufferObj *authKey, bool allocate)
{
	...
	err |= tlv_dserialize(&tlv->nonce, WPS_ID_E_SNONCE2, theBuf, SIZE_128_BITS, 0);
	err |= tlv_dserialize(&tlv->ssid, WPS_ID_SSID, theBuf, SIZE_32_BYTES, allocate);
	err |= tlv_dserialize(&tlv->macAddr, WPS_ID_MAC_ADDR, theBuf, SIZE_6_BYTES, allocate);
	err |= tlv_dserialize(&tlv->authType, WPS_ID_AUTH_TYPE, theBuf, 0, 0);
	err |= tlv_dserialize(&tlv->encrType, WPS_ID_ENCR_TYPE, theBuf, 0, 0);
	if (err)
		return WPS_ERR_GENERIC;
	...
	while (buffobj_NextType(theBuf) != WPS_ID_KEY_WRAP_AUTH) {
		if (buffobj_NextType(theBuf) == WPS_ID_WEP_TRANSMIT_KEY) {
			err |= tlv_dserialize(&tlv->wepIdx, WPS_ID_WEP_TRANSMIT_KEY, theBuf, 0, 0);
...
reg_msg_m8ap_parse(EsM8Ap *t, BufferObj *theBuf, BufferObj *authKey, bool allocate)
{
	...
	if (buffobj_NextType(theBuf) == WPS_ID_NW_INDEX) {
		if (tlv_dserialize(&t->nwIndex, WPS_ID_NW_INDEX, theBuf, 0, 0))
			goto tlv_error;
	}

	if (tlv_dserialize(&t->ssid, WPS_ID_SSID, theBuf, SIZE_32_BYTES, allocate))
		goto tlv_error;
	if (tlv_dserialize(&t->authType, WPS_ID_AUTH_TYPE, theBuf, 0, 0))
		goto tlv_error;
	if (tlv_dserialize(&t->encrType, WPS_ID_ENCR_TYPE, theBuf, 0, 0))
		goto tlv_error;
	...
	if (buffobj_NextType(theBuf) == WPS_ID_NW_KEY) {
		...
		if (!key)
			return -1;
		if (tlv_dserialize(key, WPS_ID_NW_KEY, theBuf, SIZE_64_BYTES, allocate)) {
			tlv_delete(key, 0);
			goto tlv_error;
		}
		...
		while (buffobj_NextType(theBuf) == WPS_ID_NW_KEY_INDEX) {
			...
			if (tlv_dserialize(keyIndex, WPS_ID_NW_KEY_INDEX, theBuf, 0, 0)) {
				tlv_delete(keyIndex, 0);
				goto tlv_error;
			}
	...
	if (tlv_dserialize(&t->macAddr, WPS_ID_MAC_ADDR, theBuf, SIZE_6_BYTES, allocate))
		goto tlv_error;

	if (buffobj_NextType(theBuf) == WPS_ID_NEW_PWD) {
		/* If the New Password TLV is included, the Device password ID is required */
		if (tlv_dserialize(&t->new_pwd, WPS_ID_NEW_PWD, theBuf, SIZE_64_BYTES, allocate))
			goto tlv_error;
		if (tlv_dserialize(&t->pwdId, WPS_ID_DEVICE_PWD_ID, theBuf, 0, 0))
			goto tlv_error;
	}

	if (buffobj_NextType(theBuf) == WPS_ID_WEP_TRANSMIT_KEY) {
		if (tlv_dserialize(&t->wepIdx, WPS_ID_WEP_TRANSMIT_KEY, theBuf, 0, 0))
			goto tlv_error;
	}
...
reg_msg_m8sta_parse(EsM8Sta *t, BufferObj *theBuf, BufferObj *authKey, bool allocate)
{
	...
	if (tlv_credentialParse(pCredential, theBuf, allocate)) {
		tlv_credentialDelete(pCredential, 0);
		return -1;
	}
