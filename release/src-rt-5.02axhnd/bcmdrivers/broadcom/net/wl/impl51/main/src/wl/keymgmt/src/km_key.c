...
wlc_key_rx_mpdu(wlc_key_t *key, void *pkt, d11rxhdr_t *rxh)
{
	...
	if (!(WLC_KEY_IS_LINUX_CRYPTO(&key->info))) {
		...
		if (FC_TYPE(fc) != FC_TYPE_MNG || !ETHER_ISMULTI(&hdr->a1)) {
			...
			if (PKTISHDRCONVTD(key->wlc->osh, last_pkt) && (qc & QOS_AMSDU_MASK)) {
				/* Headerconverted and AMSDU there is no ICV */
			} else {
				PKTFRAG_TRIM_TAILBYTES(KEY_OSH(key), last_pkt, key->info.icv_len,
					TAIL_BYTES_TYPE_ICV);
#ifdef BCM43684_HDRCONVTD_ETHER_LEN_WAR
				/* Correct ether lenght for type 1 conversion */
				if (PKTISHDRCONVTD(key->wlc->osh, last_pkt) &&
					...
					if (ether_type < ETHER_TYPE_MIN) {
						uint32 low_addr = PKTFRAGDATA_LO(key->wlc->osh,
							last_pkt, 1) + ETHER_TYPE_1_OFFSET;
