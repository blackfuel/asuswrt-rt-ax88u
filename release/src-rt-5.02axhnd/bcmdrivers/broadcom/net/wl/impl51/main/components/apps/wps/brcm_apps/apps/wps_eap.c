 ...
 *
 * $Id: wps_eap.c 749115 2018-02-27 20:25:46Z $
 */
	...
	char *ifname)
{
	...
	tlv_err |= tlv_dserialize(&prReqIE.rfBand, WPS_ID_RF_BAND, bufObj, 0, 0);
	tlv_err |= tlv_dserialize(&prReqIE.assocState, WPS_ID_ASSOC_STATE, bufObj, 0, 0);
	tlv_err |= tlv_dserialize(&prReqIE.confErr, WPS_ID_CONFIG_ERROR, bufObj, 0, 0);
	tlv_err |= tlv_dserialize(&prReqIE.pwdId, WPS_ID_DEVICE_PWD_ID, bufObj, 0, 0);
	if (tlv_err)
		goto err_out;
...
#ifdef BCMWPSAPSTA
		else if (event_type == WLC_E_ESCAN_RESULT) {
			wps_eap_parse_scan_result(event, ifname);
			return ret;
		}
#endif // endif
