...
wps_isWPSS(IN uint8 *p_data, IN uint32 len)
{
	...
	tlv_err |= tlv_dserialize(&probrspIE.version, WPS_ID_VERSION, bufObj, 0, 0);
	tlv_err |= tlv_dserialize(&probrspIE.scState, WPS_ID_SC_STATE, bufObj, 0, 0);

	buffobj_del(bufObj);
	if (tlv_err == 0 && probrspIE.scState.m_data == WPS_SCSTATE_CONFIGURED)
