...
static int sotp_dump_map( int * result );
static int sotp_get_row_data( int row_addr, char* sotp_data, int data_len, int * result, int raw );
static int sotp_get_keyslot_data( int section_num, char* sotp_data, int data_len, int * result );
static int sotp_set_row_data( int row_addr, char* sotp_data, int data_len, int * result, int raw );
static int sotp_set_keyslot_data( int section_num, char* sotp_data, int data_len, int * result );
static int sotp_set_keyslot_readlock(int section_num, int * result);
static int sotp_set_region_readlock(int region_num, int * result);
static int sotp_set_region_readlock(int region_num, int * result);
static int sotp_get_keyslot_readlock_status(int section_num, int * result );
static int sotp_get_region_readlock_status( int region_num, int * result );
static int sotp_get_region_fuselock_status( int region_num, int * result );
...
static int sotpLockSecKeyRead(int section_num)
{
    rdlock = 1;
    return SOTP_S_KEY_SUCCESS;
}

static int sotpWriteSecKey(int section_num,  uint32_t* sotp_data, int num_words)
{
    memcpy((char*)&key_data[0], (char*)sotp_data, num_words*sizeof(uint32_t));
    return SOTP_S_KEY_SUCCESS;
}
static int sotpReadSecKey(int section_num,  uint32_t* sotp_data, int num_words)
{
...
static SotpRowStatus sotpPollForFDone(void)
{
   ...
   reg_rd_data = SOTP->sotp_sotp_out_0;
...
static SotpRowStatus sotpSetStartAndPollForCmdDone(void)
{
   ...
   /* Set the start bit */
   SOTP->sotp_otp_ctrl_0 |= SOTP_OTP_CTRL_0_START;

   msleep(1);

   /* Wait for the FSM to say it is done */
   reg_rd_data = SOTP->sotp_otp_status_1;
...
static SotpRowStatus sotpPrepForFusing(void)
{
   ...
   SOTP->sotp_otp_prog_ctrl |= SOTP_OTP_PROG_CTRL_OTP_CPU_MODE_EN;

   /* set up the OTP_CMD for OTP_ProgEnable */
   SOTP->sotp_otp_ctrl_0 = SOTP_OTP_CTRL_0_OTP_CMD_OTP_PROG_ENABLE;

   /* Write F, 4, 8, D in sequence to BITSEL */
   for (i=0;i<4;i++)
   {
      SOTP->sotp_otp_wdata_0 = authVal[i];

      /* Start the state machine */
      if ((rval = sotpSetStartAndPollForCmdDone()) == SOTP_S_ROW_SUCCESS)
      {
         /* Clear the CMD_DONE bit */
         SOTP->sotp_otp_status_1 = SOTP_OTP_STATUS_1_CMD_DONE;
      }
      else
         return rval;
   ...
   SOTP->sotp_chip_cntrl |= (SOTP_CHIP_CTRL_SW_MANU_PROG | SOTP_CHIP_CTRL_SW_NON_AB_DEVICE | \
...
static SotpRowStatus sotpFinishFusing(void)
{
   ...
   SOTP->sotp_otp_ctrl_0 = SOTP_OTP_CTRL_0_OTP_CMD_PROG;
...
static SotpRowStatus sotpReadNonEccRow(uint32_t row, uint64_t *pDst)
{
   ...
   SOTP->sotp_otp_status_1 = (SOTP_OTP_STATUS_1_CMD_DONE | SOTP_OTP_STATUS_1_ECC_COR | SOTP_OTP_STATUS_1_ECC_DET);

   /* zero out the regs_ecc_en bits, then turn on cpu mode, and set regs_ecc_en to disabled */
   SOTP->sotp_otp_prog_ctrl &= ~( SOTP_OTP_PROG_CTRL_REGS_ECC_EN_MASK );
   SOTP->sotp_otp_prog_ctrl |=  ( SOTP_OTP_PROG_CTRL_OTP_CPU_MODE_EN | SOTP_OTP_PROG_CTRL_OTP_DISABLE_ECC | \
                              SOTP_OTP_PROG_CTRL_REGS_ECC_DIS );

   /* Set OTP addr with the row to be read */
   SOTP->sotp_otp_addr = row << SOTP_OTP_ADDR_OTP_ADDR_SHIFT;

   /* set up the OTP_CMD for OTP_READ */
   SOTP->sotp_otp_ctrl_0 = SOTP_OTP_CTRL_0_OTP_CMD_OTP_READ;

   /* Start the state machine */
   if ((rval = sotpSetStartAndPollForCmdDone()) == SOTP_S_ROW_SUCCESS)
   {
      /* Data in row is valid. Pass it to return buffer */
      *pDst = (((uint64_t)(SOTP->sotp_otp_rdata_1) << 32) | (SOTP->sotp_otp_rdata_0));
...
static SotpRowStatus sotpFuseNonEccRow(uint32_t row, uint64_t src)
{
   ...
   SOTP->sotp_otp_prog_ctrl &= ~(SOTP_OTP_PROG_CTRL_OTP_ECC_WREN);

   /* Set OTP addr with the row to be fused */
   SOTP->sotp_otp_addr = row << SOTP_OTP_ADDR_OTP_ADDR_SHIFT;

   /* Set up the registers with the data to be written */
   SOTP->sotp_otp_wdata_0 = (uint32_t)(src & 0xffffffff); /* lower 32 bit data word */
   SOTP->sotp_otp_wdata_1 = (uint32_t)(src >> 32); /* upper 32 bit data word */
...
static SotpRowStatus sotpWriteEccRow(uint32_t row, uint32_t src, uint32_t raw_write)
{
   ...
   if( raw_write )
   {
      ...
      SOTP->sotp_otp_prog_ctrl |= SOTP_OTP_PROG_CTRL_OTP_DISABLE_ECC;
      SOTP->sotp_otp_prog_ctrl &= ~(SOTP_OTP_PROG_CTRL_OTP_ECC_WREN);
   }
   else
   {
   ...
   /* Set up the registers with the data to be written */
   SOTP->sotp_otp_wdata_0 = src; /* 32 bit data word */
   SOTP->sotp_otp_wdata_1 = 0;   /* ensure the two fail bits are not fused */
...
static SotpRowStatus sotpFuseSecKeyRowFailBits(uint32_t row)
{
   ...
   /* Set up the registers with the data to be written */
   SOTP->sotp_otp_wdata_0 = 0;                          /* 32 bit data word  ... no change    */
   SOTP->sotp_otp_wdata_1 = SOTP_OTP_WDATA_1_FAIL_MASK; /* ensure the two fail bits are fused */
...
static SotpRowStatus sotpRegionReadLockStatus(uint32_t region)
{
   ...
   if (SOTP->sotp_otp_rd_lock & ( 0x1 << region )) 
...
static SotpRowStatus sotpReadEccRow(uint32_t row, uint32_t *pDst, uint32_t raw_read)
{
   ...
   SOTP->sotp_otp_prog_ctrl &= ~( SOTP_OTP_PROG_CTRL_OTP_DISABLE_ECC | SOTP_OTP_PROG_CTRL_REGS_ECC_EN_MASK );

   /* Disable ECC checks for raw reads */
   if( raw_read )
      SOTP->sotp_otp_prog_ctrl |= ( SOTP_OTP_PROG_CTRL_OTP_CPU_MODE_EN | SOTP_OTP_PROG_CTRL_OTP_DISABLE_ECC | SOTP_OTP_PROG_CTRL_REGS_ECC_DIS );
   else
      SOTP->sotp_otp_prog_ctrl |= ( SOTP_OTP_PROG_CTRL_OTP_CPU_MODE_EN | SOTP_OTP_PROG_CTRL_REGS_ECC_EN );
   ...
   if ((rval = sotpSetStartAndPollForCmdDone()) == SOTP_S_ROW_SUCCESS)
   {
      ...
      if ( SOTP->sotp_otp_rdata_1 & SOTP_OTP_RDATA_1_FAIL_MASK )
      {
      ...
      if ( SOTP->sotp_otp_status_1 & SOTP_OTP_STATUS_1_ECC_DET )
      {
      ...
      if ( SOTP->sotp_otp_status_1 & SOTP_OTP_STATUS_1_ECC_COR )
      {
...
/*  *********************************************************************
    ...
    *
    ********************************************************************* */
static SotpKeyStatus sotpReadSecKey(uint32_t section, uint32_t *pDst, uint32_t len)
{
   ...
   SOTP_DBG("%s: section:%d bufp:%p len:%d\n", __FUNCTION__, section, pDst, len);
...
static SotpKeyStatus sotpReadLockRegion(uint32_t region)
{
   ...
   /* expect Bug?  When ecc=0 then programs 0x0, Reg says that it should be set to 4'b0101 : ECC check DISABLED */
   SOTP->sotp_otp_prog_ctrl &= ~SOTP_OTP_PROG_CTRL_REGS_ECC_EN_MASK;
   SOTP->sotp_otp_prog_ctrl |=  ( SOTP_OTP_PROG_CTRL_OTP_CPU_MODE_EN | SOTP_OTP_PROG_CTRL_REGS_ECC_DIS );
   ...
   SOTP->sotp_otp_rd_lock = 1 << region;
...
/*  *********************************************************************
    ...
    *
    ********************************************************************* */
static SotpKeyStatus sotpLockSecKeyRead(uint32_t section)
{
   ...
   /* The read lock register is a write-once register on a bit-for-bit bassis     */
   /* so you don't have to worry about preserving bits that are already set to 1. */
   SOTP->sotp_otp_rd_lock = SOTP_REGIONS_MASK_IN_KEYSLOT << startRegion;
...
static SotpKeyStatus sotpKeySlotReadLockStatus(uint32_t section)
{
   ...
   for( i=0; i<SOTP_REGIONS_IN_KEYSLOT; i++ )
   {
       ...
       /* Check to see if that region is locked */
       if (SOTP->sotp_otp_rd_lock & ( 0x1 << startRegion )) 
          return SOTP_E_ROW_READ_LOCK;
...
/*  *********************************************************************
    ...
    *
    ********************************************************************* */
static SotpKeyStatus sotpWriteSecKey(uint32_t section, uint32_t *pSrc, uint32_t len)
{
...
static void dump_regions(int first_region, int last_region, int section, int regions_in_section )
{
    ...
    for( i=first_region; i<last_region; i++ )
    {
        ...
        fuse_lock = sotpIsRegionFuseLocked(i);
        read_lock = sotpRegionReadLockStatus(i);
        printk("        Region[%d]: Fuselock=%d Readlock=%d\n",
                i,
...
static int sotp_dump_map( int * result )
{
    ...
    printk("Wrapper/Prog LOCK SOTP Sections:\n");
    ...
    printk("Device Config SOTP Section:\n");
    ...
    printk("General Config SOTP Sections:\n");
...
//    section = SOTP_FIRST_CFG_SECTION;
//    first_region = SOTP_FIRST_ECC_CONFIG_ROW/SOTP_ROWS_IN_REGION;
//    last_region = SOTP_FIRST_KEYSLOT_ROW/SOTP_ROWS_IN_REGION;
//    printk("General Config SOTP Sections:\n");
//    dump_regions(first_region, last_region, section, SOTP_REGIONS_IN_CFG);
//
...
{
    int i=0;
    *result = sotpReadSecKey(section_num, (uint32_t*)sotp_data, data_len/sizeof(uint32_t));
...
{
    int i=0;
    *result = sotpWriteSecKey(section_num, (uint32_t*)sotp_data, data_len/sizeof(uint32_t));
    SOTP_DBG("%s: addr:%d, len:%d, res:%d, value:\n", __FUNCTION__, section_num, data_len, *result);
    for( i=0; i<data_len/sizeof(uint32_t); i++ )
    {
        SOTP_DBG("0x%08x\n", *((uint32_t *)sotp_data+i));
...
static int sotp_set_region_fuselock(int region_num, int * result )
{
    SOTP_DBG("%s: addr:%d\n", __FUNCTION__, region_num);
...
static int sotp_set_keyslot_readlock(int section_num, int * result )
{
    ...
    *result = sotpLockSecKeyRead(section_num);
